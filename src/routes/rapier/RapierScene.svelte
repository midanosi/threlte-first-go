<script lang="ts">
	...
	useFrame(() => {
		...
		playerMesh.getWorldPosition(p3);
		cubeMesh.getWorldPosition(c3);
		smoothPlayerPosX.set(p3.x, {
			hard: !positionHasBeenSet
		});
		smoothPlayerPosZ.set(p3.z, {
			hard: !positionHasBeenSet
		});
...
<CollisionGroups groups={[0]}>
	...
	<!-- cube -->
	<T.Mesh bind:ref={cubeMesh} position={[1, 2, 1]}>
		<T.MeshStandardMaterial
			color={p3.sub(c3) < 5 ? 'hotpink' : 'gold'}
		/>
		<T.BoxGeometry />
	</T.Mesh>
